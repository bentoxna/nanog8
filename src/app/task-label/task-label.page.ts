...
@Component({
  ...
  templateUrl: './task-label.page.html',
  styleUrls: ['./task-label.page.scss'],
})
export class TaskLabelPage implements OnInit {
  ...
  //       // });
  //     } else if(event.target.files && event.target.files[i] && event.target.files[i].size >= (10485768)) {
  //       // Swal.fire({
  ...
  //       Swal.fire({
  //         title: 'processing...',
  //         text: 'The detected image exceeds the 8MB limit, which may result in a longer upload time.',
  //         icon: 'info',
  //         heightAuto: false,
  //         allowOutsideClick: false,
  //         showConfirmButton: false,
